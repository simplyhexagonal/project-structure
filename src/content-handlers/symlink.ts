...
export const symlinkContentHandler: ContentSourceHandler = async (
  {
  ...
  try {
    ...
    const {
      ...
      `cd ${from} && for f in $(find . -mindepth 1 -maxdepth 1 | cut -c 3-); do ln -s $(pwd)/$f ${to}$f; done`,
    );
    ...
    return {
      success: false,
      message: `failed to create symbolic link to content from ${from} to ${to}`,
    };
  }

  return {
    success: true,
    message: `synced create symbolic link to content from ${from} to ${to}`,
  };
}
